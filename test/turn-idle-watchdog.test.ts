...
test('idle watchdog resets on every notification — long turn with activity does not fail', async () => {
  ...
  await withEnv('CLI_CODEX_WORKER_STATE_DIR', stateDir, async () => {
    await withEnv('CODEX_HOME_DIRS', '/tmp/fake-codex-home', async () => {
      await withEnv('CODEX_WORKER_TURN_TIMEOUT_MS', '200', async () => {
        const fake = new NotificationStreamFakeClient('/tmp/workspace', '/tmp/fake-codex-home');
        // Heartbeat every 80 ms for 500 ms — 6 notifications, none silent for the full 200 ms window.
        fake.schedule = [80, 160, 240, 320, 400, 480].map((delayMs) => ({
          delayMs,
          method: 'item/agentMessage/delta',
          params: { delta: 'x' },
        }));
        // Then complete cleanly at 560 ms.
        fake.schedule.push({
          delayMs: 560,
        ...
        const localThread = snapshot.localThread as { status: string };
